...
pub fn main() void {
    ...
    while (win_it.next()) |s| {
        // Check string is long enough for shortest op = "do()".
        if (s.len < do.len) continue;

        // Handle the "do()" and "don't()" commands before trying to parse a "mul()".
        if (std.mem.eql(u8, s[0..do.len], do)) {
            enabled = true;
            continue;
        }

        if (std.mem.eql(u8, s[0..dont.len], dont)) {
            enabled = false;
            continue;
        }
